...
exports.findFolderMap = async (userid, folderid) =>
  folderid
    ? await query(
        `SELECT * FROM folders WHERE OWNER ='${userid}' AND parentfolderid='${folderid}'`
      )
    : await query(
        `SELECT * FROM folders WHERE OWNER ='${userid}' AND ISNULL(parentfolderid)`
      );
